...
# Modified get_order func
def get_order(n_samples):
    filepath_prefix = "C:\\Users\\adevr\\MITx_6.419x\\week_2\\sentiment_analysis\\"
    try:
        with open(filepath_prefix + str(n_samples) + '.txt') as fp:
            line = fp.readline()
            return list(map(int, line.split(',')))
    except FileNotFoundError:
        print("FILE NOT FOUND")
...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    if z > 0:  # if correct classification, return 0
        return 0
    else:
        return 1 - z
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    n = feature_matrix.shape[0]  # number of rows
    p = feature_matrix.shape[1]  # number of cols
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    if label * (np.dot(current_theta, feature_vector) + current_theta_0) > 0:  # if classification correct
        theta = current_theta
...
def perceptron(feature_matrix, labels, T):
    ...
    # Initialize weights vector theta
    theta = np.zeros(p)
    theta_0 = 0
...
def pegasos(feature_matrix, labels, T, L):
    ...
    # Initialize weights vectors theta, theta_0
    theta = np.zeros(p)
    theta_0 = 0
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    pred = feature_matrix @ theta + theta_0
...
def classifier_accuracy(
    ...
    acc_train = accuracy(preds_train, train_labels)
    acc_val = accuracy(preds_val, val_labels)
