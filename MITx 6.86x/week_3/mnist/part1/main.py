...
def closed_form(X, Y, lambda_factor):
    ...
    n = X.shape[0]
    d = X.shape[1] - 1
    theta = np.linalg.inv(X.T @ X + lambda_factor * np.eye(d+1)) @ X.T @ Y
    return theta
...
# print('SVM one vs. rest test_error, C=1', run_svm_one_vs_rest_on_MNIST(C=1))
# print('SVM one vs. rest test_error, C=0.1', run_svm_one_vs_rest_on_MNIST(C=0.1))
# print('SVM one vs. rest test_error, C=0.01', run_svm_one_vs_rest_on_MNIST(C=0.01))
...
def run_softmax_on_MNIST(temp_parameter=1):
    ...
    print("    train_x_centered.shape =", train_x_centered.shape)
    pcs = principal_components(train_x_centered)
    train_x_projected_cubic = cubic_features(project_onto_PC(train_x, pcs, 10, feature_means))
    print("    train_x_projected.shape =", train_x_projected_cubic.shape)

    theta, cost_function_history = softmax_regression(train_x_projected_cubic, train_y, temp_parameter, alpha=0.3, lambda_factor=1.0e-4, k=10, num_iterations=150)
    ...
    # print("test_error_mod3:", compute_test_error_mod3(test_x, test_y, theta, temp_parameter))
    
    return test_error

run_softmax_on_MNIST(temp_parameter=1)
# print('softmax test_error with temp_parameter=1 =', run_softmax_on_MNIST(temp_parameter=1))
# print('softmax test_error with temp_parameter=0.5 =', run_softmax_on_MNIST(temp_parameter=0.5))
# print('softmax test_error with temp_parameter=2 =', run_softmax_on_MNIST(temp_parameter=2))
...
# print("test error for mod 3 =", run_softmax_on_MNIST_mod3(temp_parameter=1))
